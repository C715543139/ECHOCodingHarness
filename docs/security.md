...
当前默认值为 128 个 Step、300 秒工具超时、80,000 字符工具输出上限、600 秒 Provider
请求超时、256,000 近似 token 上下文和 16,000 输出 token 预留；配置契约见
[contracts.md](./contracts.md)。这些较宽松但仍有限的预算用于复杂任务与录制演示；第 10 次等价工具
请求仍会在执行前被拒绝。超时、取消、安全策略、工作区边界和 Full Access 人工确认不受预算放宽影响。
