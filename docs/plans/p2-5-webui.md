...
- 输入区模型与安全模式选择器固定为 `2rem` 高，使用受控的 `1.25rem` 行高、上下 `0.3125rem`
  内边距与自绘下拉标记，规避 Windows 原生 select 基线偏下；
...
实机验收发现提交 Turn 后客户端会重新加载当前 Session 并主动替换仍然健康的 SSE；新旧连接释放
存在短暂竞争，顶栏因而错误显示“未连接 / 正在重连”。Trace 初次进入时虽然处于尾随模式，却没有
把滚动位置应用到最新事件。旧的“跳到主内容”入口也不再符合最终精简界面。
