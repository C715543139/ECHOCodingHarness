...
同日录制排练进一步选择 `google/gemini-3.7-flash` 作为推荐演示模型。它通过正文落盘 smoke，并在
当前演示默认的 128 Step、15 分钟总硬超时下约两分钟完成同一组严格证据。
排练提示词只增加“测试通过后停止可选改进、不要重复结果相同的未变命令”的效率约束，仍不提供扩展
ID、工具名或生命周期顺序。默认配置保持 `balanced`；进入演示任务时仍需由人类显式确认 Full Access。
