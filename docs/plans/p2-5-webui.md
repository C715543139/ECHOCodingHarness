...
浅色重构通过后，桌面实测仍发现侧栏宽度、工作区名称可读性、顶栏对齐、页面滚动所有权、设置模型
列表和“回到最新”提示不够一致。
...
- 工作区行增大脱敏名称字号并允许长名称换行，悬停仍显示完整脱敏名称。Web DTO 继续禁止 `/`、`\`
  和盘符，绝对路径不得为解决显示问题而进入浏览器；
- 桌面顶栏使用三列布局，将“对话 / 轨迹”稳定置中，连接状态保持右对齐；
- 桌面主壳固定为视口高度并隐藏文档级溢出，滚动所有权只属于 Chat、Trace、Session 列表、Inspector
  和设置内容等明确区域；窄屏仍使用可访问的单列文档流；
- 自动发现与手动维护的模型使用最高 `11rem` 的独立键盘可聚焦滚动列表，不再随模型数量撑高设置卡；
- 输入区模型与安全模式选择器使用显式行高和内边距，保持文字垂直居中；
- Chat 与 Trace 的尾随恢复统一为“回到最新”浮层按钮。Chat 位于输入区上方，Trace 位于列表底部；
  浮层容器不绘制额外背景，也不占用内容布局高度。

### 验收证据

- `tests/unit/web/session-rail.test.tsx` 覆盖拖动、键盘边界和恢复默认宽度；
- `tests/unit/web/chat-stream.test.tsx` 与 `tests/unit/web/inspector.test.tsx` 覆盖统一文案、浮层位置和
  恢复尾随；
- `tests/e2e/web/session-flow.spec.ts` 覆盖桌面拖动、文档无整体滚动和模型列表独立滚动样式；
