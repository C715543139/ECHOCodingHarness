...
- `pnpm accept:p3-pdf` 使用 `.env.test` 的真实 OpenAI-compatible Provider 显式验收，临时工作区在结束后
  ...
  JSONL 中判定复用，避免扫描全部历史 Session 造成假阳性。演示友好的默认首轮上限为 128 步、
  复用轮仍为 4 步，两轮共享 15 分钟总硬超时；失败时只报告有界工具名，不回放模型正文。录制前的显式本地排练可用
  `ECHO_ACCEPT_P3_MAX_STEPS`（最大 160）和 `ECHO_ACCEPT_P3_TIMEOUT_MS`（最大 20 分钟）临时放宽首轮
  Step 与总超时，不改变复用轮 4 Step 和其他可信证据门槛，也不影响 CI。产品默认同时采用 128 Step、
  300 秒工具超时、80,000 字符工具结果、600 秒 Provider 请求超时和第 10 次等价工具请求停止；安全边界不变。
