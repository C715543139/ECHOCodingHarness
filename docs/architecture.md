...
本文定义 ECHO Harness 首个可交付版本的架构边界与关键约束。它描述系统应如何组织，而不把尚未实现的细节写成既成事实。实现与本文冲突时，应先更新相应 ADR，再修改本文。
...
- PowerShell 以非交互方式启动，不加载用户 Profile；使用 `-InputFormat Text` 并立即关闭 stdin，避免无控制台会话中的 CLIXML 等待；
- 子进程使用显式工作目录和净化后的环境变量，模块路径仅指向系统 `WindowsPowerShell\v1.0\Modules`；
...
具体命令和覆盖矩阵在实现后写入 `docs/testing.md`。
...
出现时终止。会话恢复仍不进入 P0；真实 Provider 的服务兼容性继续通过显式本地验收测试
验证，不进入 CI。
