...
- 分页列出固定工作区 Session；
- 恢复并切换 Session；
- 桌面在 208–420 px 内调节侧栏宽度，默认 280 px；
- 显示脱敏标题、更新时间、模型，以及不加图标的文字状态；
- 工作区行使用“当前工作区”标签和较大的脱敏名称，不显示文件夹图标；具体 Session 标题与“新会话”
  按钮使用相同字号；工作区脱敏名称在侧栏可用宽度内居中；
...
- Chat 阅读区使用独立的响应式四周留白；相同的 Turn 状态与 stop reason 只显示一次，不同原因仍
  保留；
- Chat 主信息与输入卡共用最大 `56rem` 的居中内容列，桌面宽屏不把用户气泡和模型正文推向两端；
- 聚合 `model.text` 支持安全的 CommonMark/GFM，用户输入仍按纯文本显示；远程图片不自动加载；
- 当前直播使用 SSE 更新同一条记录，刷新只读聚合 Session 事实；
- 同一 Session 提交 Turn 时复用健康 SSE，不因提交成功而显示未连接或重连；
...
- 用户上滚后暂停自动跟随，使用底部“回到最新”浮层恢复尾随；
- 文档根节点不滚动，Chat/Trace 只开放各自的纵向内部滚动；
...
- Base URL；
- 自动发现或手动模型目录，二选一；发现结果是独立滚动的只读列表，不作为当前 Session 模型选择器；
...
Trace 不使用顶部图形时间线。记录按 Session seq 从旧到新排列，并按 Turn/Step 轻量分组。进入 Trace
或切换 Session 时默认尾随并定位到最新记录；用户主动上滚后才暂停尾随。固定类型如下：
