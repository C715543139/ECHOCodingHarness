...
3. **事件先于界面**：CLI、日志和未来 UI 消费同一事件模型，核心逻辑不依赖表现层。
...
CLI 负责参数解析、交互审批、事件渲染和退出码，不包含 Agent 决策逻辑。CI 和演示烟测必须可以通过非交互参数运行。
...
以下内容在代码完成前保持开放，不在 v0.1 中伪造精确答案：

- 最终目录结构与包边界；
- API 的具体兼容差异与重试参数；
- 事件字段、错误码与 CLI 退出码的最终枚举；
- 上下文预算默认值与截断阈值；
- 危险命令规则的完整覆盖范围；
- 会话恢复是否进入首个交付版本。
