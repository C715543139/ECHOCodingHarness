...
| `session.resumed` / `model.changed` / `safety.changed` | `run` 默认无输出；Chat 启动摘要与 Slash 反馈由 Chat 表现层渲染。`--resume` 接受摘要中的唯一 SESSION 短 ID；空白或非法标识（含 `../` 与 `..\\`）以退出码 2 失败 |
